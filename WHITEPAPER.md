...
```text
HR legal/workforce truth:
NhanVien -> HopDongLaoDong / HoSoBaoHiem / DonNghiPhep / QuyetDinhNghiViec / OffboardingChecklist

Scheduling truth:
ShiftChangeRequest -> approval -> APPLIED -> PhanCongCaTruc

Payroll source truth:
TamUngLuong / KhoanKhauTruNhanVien / DonNghiPhep / HoSoBaoHiem -> PayrollSourceReconciliationUseCase

Customer receivable truth:
HoaDon / CongNo -> ThanhToanKhachHang -> PhanBoThanhToanHoaDon -> receivable status

Asset recovery truth:
PhieuXuat -> PhieuThuHoi -> InventoryLedgerEntry / BienBanMatHongVatTu -> KhoanKhauTruNhanVien
```
...
- Sau khi kỳ lương đã `LOCKED` hoặc `PAID`, thay đổi hồi tố không được sửa trực tiếp `ChiTietLuong`; phải tạo `PayrollAdjustment` hoặc reversal record có kiểm soát, có nguồn dữ liệu, lý do, người duyệt và audit.

### 6.3 Customer receivable reconciliation

- `CongNo.so_tien_da_thu` is derived/synchronized from `PhanBoThanhToanHoaDon`; it is not a source field after Phase E.
- Customer payment/allocation mutation must go through `CustomerPaymentPermissionPolicy` and application use cases.
- Allocated payment/allocation source fields are append-only. Correction requires explicit reversal/adjustment records in a later phase.
- Invoice-only allocation is invalid; allocation must target a concrete `CongNo` so `HoaDon` and `CongNo` cannot diverge.
...
- `PhieuThuHoi` is the source record for employee asset return. Do not use fake `PhieuNhap` to represent offboarding recovery.
- Returned-good quantity creates ledger `IN`; void creates reversal `OUT`.
...
Canonical state rules are detailed in `docs/BUSINESS_WORKFLOW_A_TO_F_SYSTEM_CONTRACT.md`.

Minimum non-negotiable states:

```text
HopDongLaoDong: DRAFT -> PENDING_SIGNATURE -> ACTIVE/EXPIRING/EXPIRED/TERMINATED
ShiftChangeRequest: DRAFT -> PENDING_APPROVAL -> APPROVED -> APPLIED
ThanhToanKhachHang: DRAFT -> RECEIVED -> ALLOCATED / CANCELLED
PhieuThuHoi: DRAFT -> POSTED -> VOIDED
BienBanMatHongVatTu: DRAFT -> PENDING_APPROVAL -> APPROVED -> APPLIED / CANCELLED
```
...
Archive strategy must preserve referential traceability for payroll, incident, inventory and attendance disputes. Historical scope checks must keep working after archive transitions.
...
The following files are authoritative companion contracts for Phase 1 Access Scope hardening:
...
Django Admin/Jazzmin, sensitive exports, migration/backfill and UI surface changes must not proceed without checking these contracts.
