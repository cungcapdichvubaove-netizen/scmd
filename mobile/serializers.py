...
<<<<<<< HEAD
        ...
        # Trường hợp Admin/Superuser: Thấy toàn bộ mục tiêu của tổ chức
        if user.is_staff or user.is_superuser:
            self.fields['muc_tieu_id'].queryset = MucTieu.objects.for_tenant(tenant_id)
            self.fields['ca_truc_id'].queryset = PhanCongCaTruc.objects.for_tenant(tenant_id)
            return

        if nhan_vien is None:
            return

        # Trường hợp Nhân viên: Chỉ thấy các ca trực và mục tiêu mình được phân công (Operational Truth)
        assigned_shifts = PhanCongCaTruc.objects.for_tenant(tenant_id).filter(nhan_vien=nhan_vien).select_related('vi_tri_chot__muc_tieu')
